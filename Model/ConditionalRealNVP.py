...
def ConditionalCoupling(input_shape,conditional_shape):
    ...
    for n in [output_dim,2*output_dim,2*output_dim]:
        t_out = tf.keras.layers.Dense(n,activation='relu',kernel_regularizer=regularizers.l2(reg))(t_out)
    ...
    for n in [output_dim,2*output_dim,2*output_dim]:
        s_out = tf.keras.layers.Dense(n,activation='relu',kernel_regularizer=regularizers.l2(reg))(s_out)
...
class ConditionalRealNVP(tf.keras.Model):
    def __init__(self,num_coupling_layers,dim,ncond):
    ...
    def log_loss(self, inputs):
        ...
        log_likelihood = self.distribution.log_prob(y+1e-6) + logdet
